...
export default function AdminDashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col md:flex-row text-slate-800 antialiased font-sans">
      ...
      {/* Sidebar */}
      <aside className={`${isMobileMenuOpen ? 'block' : 'hidden'} md:flex fixed md:static inset-0 z-40 w-full md:w-64 bg-white border-r border-slate-200/80 flex-col justify-between flex-shrink-0 p-5 md:min-h-screen pt-20 md:pt-5`}>
        <div>
        ...
        <div className="pt-6 space-y-3 border-t border-slate-100 mt-auto md:mt-0">
          <div className="bg-emerald-50/90 border border-emerald-200/70 px-3.5 py-2.5 rounded-xl flex items-center gap-2.5 shadow-sm">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
            </span>
            <span className="text-[11px] font-extrabold text-emerald-700 tracking-tight">Supabase DB Connected</span>
          </div>
